...
import { Upload, User, LogOut, Bell, Mail, Smartphone } from 'lucide-react'
...
export default function Navigation() {
  ...
  const { notifications: _notifications, unreadCount, markAsRead: _markAsRead, markAllAsRead: _markAllAsRead } = useNotifications()
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 sm:h-18">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 md:gap-2.5">
            ...
            <Button asChild className="bg-indigo-600 hover:bg-indigo-700 text-white h-9 sm:h-10 px-4 sm:px-5 text-sm font-medium transition-all duration-200 hover:scale-105" title="Upload study materials">
            ...
            {loading ? (
              ...
              <>
                ...
                <Dialog open={showAuthDialog} onOpenChange={setShowAuthDialog}>
                  <DialogContent className="max-w-md mx-auto w-[calc(100%-2rem)] sm:w-full max-h-[90vh] overflow-y-auto my-auto bg-white border-2 border-indigo-100">
                    <DialogHeader>
                      <div className="flex flex-col items-center gap-4 mb-4">
                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-700 p-4 shadow-xl flex items-center justify-center ring-4 ring-indigo-100 transition-transform hover:scale-105">
                          <svg viewBox="0 0 24 24" fill="white" className="w-full h-full">
                            <path d="M12 3L1 9l11 6 9-4.91V17h2V9M5 13.18v4L12 21l7-3.82v-4L12 17l-7-3.82z"/>
                          </svg>
                        </div>
                    ...
                    <div className="flex gap-2 bg-gradient-to-r from-gray-100 to-gray-50 p-1.5 rounded-xl shadow-inner">
                      ...
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-semibold transition-all ${
                          authTab === 'phone'
                            ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg ring-2 ring-purple-200'
                            : 'text-gray-600 hover:text-gray-900 hover:bg-white/50'
                        }`}
                      >
                    ...
                    {authTab === 'email' && (
                      <form onSubmit={handleSignIn} className="space-y-5 pt-3">
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <Input
                              ...
                              required
                              className="h-14 text-base pl-12 border-2 border-gray-200 focus:border-indigo-400 rounded-xl shadow-sm"
                            />
                          </div>
                        </div>
                        <Button 
                          type="submit" 
                          className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 h-14 text-base font-bold rounded-xl shadow-lg hover:shadow-xl transition-all" 
                        ...
                        <Button
                          type="button"
                          variant="outline"
                          className="w-full border-2 border-gray-300 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 hover:border-indigo-300 h-14 font-semibold transition-all rounded-xl shadow-md hover:shadow-lg"
                          onClick={handleGoogleSignIn}
                          disabled={authLoading}
                        >
                          {authLoading ? (
                            <div className="flex items-center gap-3">
                              <div className="w-6 h-6 animate-spin border-3 border-blue-500 border-t-transparent rounded-full"></div>
                              Signing in...
                            </div>
                          ) : (
                        ...
                        {authMessage && (
                          <div className={`text-sm text-center p-3 rounded-lg ${authMessage.includes('error') || authMessage.includes('Error') ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>
