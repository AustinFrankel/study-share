      ...
      >
        ...
        <CardContent className="space-y-3 sm:space-y-4 px-4 sm:px-6 pb-4 sm:pb-6 flex-1 flex flex-col">
          ...
          >
            {isPast ? (
              <>
                <span className="truncate">{test.hasResources ? 'View' : 'Coming soon'}</span>
              </>
            ) : (
              <>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 md:py-10">
        ...
        <div className="text-center mb-6 sm:mb-8 md:mb-10">
          ...
          <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl mx-auto px-4">
            View tests before they are released.
          </p>

          {/* Search row with Past Tests button on right */}
          <div className="mt-6 flex items-center justify-center gap-4 px-4">
            <div className="flex-1 max-w-3xl">
              <SearchBar placeholder="Search for tests, practice materials..." />
            </div>
            <Button
              onClick={() => router.push('/live/past')}
              variant="outline"
              className="bg-white hover:bg-indigo-50 border-2 border-indigo-300 text-indigo-700 font-semibold whitespace-nowrap h-11 sm:h-14 px-4 sm:px-6 rounded-xl"
            >
        ...
        {/* Removed duplicate search bar above Active Tests per UX cleanup */}
        ...
        <section className="mb-10 sm:mb-12">
          ...
          {/* Connect white content to green header: only bottom corners rounded and no top border */}
          <div className="bg-white rounded-b-2xl shadow-lg p-6 border border-gray-100 border-t-0">
            {(() => {
              // Group tests by name (SAT, ACT, PSAT, AP when in 31-day window)
              const groupedByName: Record<string, TestDate[]> = {}
              const allBase: TestDate[] = [
                ...categorizedTests.priority,
                ...categorizedTests.ap
              ]
              allBase.forEach(test => {
              ...
              // Show all upcoming tests in a horizontal layout (within 31 days)
              const allUpcomingTests = Object.values(groupedByName)
                .flat()
                .filter(test => {
                  const now = new Date()
                  const testDate = test.date
                  const daysUntil = Math.ceil((testDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
                  return daysUntil <= 31 && daysUntil >= 0 // Only show tests within 31 days and not past
                })
              
              return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                  {allUpcomingTests.map(test => (
        ...
        {/* Removed AP Exams Section - AP items are included in Active/Past via 31-day rule */}
                              ...
                              {hasMultiple && !isExpanded && (
                                <div className="flex justify-start mt-1">
                                  <Button
                                    ...
                                    variant="outline"
                                    className="border-2 border-dashed border-gray-400 hover:border-indigo-500 hover:bg-indigo-50 transition-all duration-200 px-4 py-2 text-sm"
                                  >
