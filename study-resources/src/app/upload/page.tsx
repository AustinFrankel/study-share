...
export default function UploadPage() {
  ...
  const [accountSetupTimeout, setAccountSetupTimeout] = useState(false)

  console.log('UploadPage render:', { user: !!user, loading, session: !!session })

  // Add timeout for "Setting up your account..." screen
  useEffect(() => {
    if (session && !user && !loading) {
      const timeout = setTimeout(() => {
        console.warn('⚠️ Account setup timed out, redirecting to login')
        setAccountSetupTimeout(true)
  ...
  if (session && !user && !accountSetupTimeout) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
        <Navigation />
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  ...
  // If account setup timed out, show error and option to retry
  if (accountSetupTimeout && session && !user) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
        <Navigation />
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="max-w-md mx-auto text-center py-12 bg-white rounded-xl shadow-lg border border-red-100 px-8">
            <div className="bg-red-100 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
              <Lock className="w-10 h-10 text-red-600" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900 mb-3">Account Setup Issue</h3>
            <p className="text-gray-600 mb-6">
              We're having trouble setting up your account. This might be a temporary connection issue.
            </p>
            <div className="space-y-3">
              <Button
                onClick={() => {
                  setAccountSetupTimeout(false)
