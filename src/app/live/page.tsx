    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 md:py-10">
        ...
        {filteredTests.length === 0 ? (
          <Card className="text-center py-12">
            <CardContent>
              <p className="text-gray-500 text-base sm:text-lg">No tests found. Try adjusting your filters or search query.</p>
            </CardContent>
          </Card>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
            {filteredTests.map((test) => {
              ...
              return (
                <Card 
                  key={test.id}
                  className="overflow-hidden hover:shadow-2xl transition-all duration-300 sm:transform sm:hover:-translate-y-2 cursor-pointer border-2 hover:border-indigo-300 bg-white"
                    ...
                    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-3 shadow-inner">
                      ...
                      {mounted && countdown && !isPast ? (
                        ...
                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-lg p-3 sm:p-4 text-center">
                          ...
                          <span className="text-blue-700 font-semibold text-xs sm:text-sm">Test Date Passed</span>
                        </div>
                      ) : (
        ...
        <div className="mt-10 sm:mt-12 md:mt-16 bg-white rounded-xl shadow-lg p-5 sm:p-6 md:p-8 border-2 border-indigo-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 md:gap-8 mt-4 sm:mt-6 md:mt-8">
            ...
            <div className="text-center p-4 sm:p-5 md:p-6 bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl">
              ...
              <h3 className="font-semibold text-lg sm:text-xl mb-2 sm:mb-3">Real-Time Updates</h3>
              <p className="text-gray-600 text-xs sm:text-sm">Live countdowns keep you informed and prepared for every upcoming test</p>
            </div>
          </div>
        </div>
      </main>
      ...
      <Dialog open={showWaitlistDialog} onOpenChange={setShowWaitlistDialog}>
        <DialogContent className="max-w-md mx-4 sm:mx-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl sm:text-2xl flex-wrap">
              {selectedTest && mounted && (
                <>
                  <span className="text-2xl sm:text-3xl">{selectedTest.icon}</span>
                  <span className="break-words">Join {selectedTest.name} Waitlist</span>
                </>
              )}
            </DialogTitle>
          </DialogHeader>
          ...
          {!submitted ? (
            <form onSubmit={handleWaitlistSubmit} className="space-y-3 sm:space-y-4 mt-3 sm:mt-4">
              {selectedTest && (
                <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4">
                  <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-700 mb-2">
                    ...
                    {selectedTest.time && <span className="hidden sm:inline">• {selectedTest.time}</span>}
                  </div>
                  {selectedTest.time && (
                    <div className="text-xs text-gray-500 mb-2 sm:hidden">{selectedTest.time}</div>
                  )}
              ...
              <Button
                type="submit"
                disabled={loading}
                className={`w-full ${selectedTest ? `bg-gradient-to-r ${selectedTest.color}` : 'bg-indigo-600'} hover:opacity-90 text-white font-semibold py-5 sm:py-6 text-sm sm:text-base`}
              >
                {loading ? (
            ...
            <div className="text-center py-6 sm:py-8">
              ...
              <Button
                ...
                className="bg-indigo-600 hover:bg-indigo-700 py-5 sm:py-6 px-6 sm:px-8"
              >
                Done
              </Button>
            </div>
