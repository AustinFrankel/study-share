...
function SearchBarContent({ placeholder = "Search resources, classes, teachers...", className }: SearchBarProps) {
...
export default function SearchBar(props: SearchBarProps) {
  return (
    <Suspense fallback={
      <form className={`flex gap-2 sm:gap-3 ${props.className}`}>
        <div className="relative flex-1 rounded-xl sm:rounded-2xl bg-white/90 backdrop-blur shadow-sm">
          <Search className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5" />
          <Input
            type="text"
            placeholder={props.placeholder || "Search resources, classes, teachers..."}
            disabled
            className="pl-10 sm:pl-12 h-11 sm:h-14 text-sm sm:text-base md:text-lg rounded-xl sm:rounded-2xl border-2 border-gray-200"
          />
        </div>
        <Button type="submit" disabled className="h-11 sm:h-14 px-4 sm:px-6 text-sm sm:text-base rounded-xl whitespace-nowrap">
          Search
        </Button>
      </form>
    }>
